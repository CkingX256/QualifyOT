...
def _residual_flow_cached(source_tuple,target_tuple,b_shape,b_bytes,tiny):
    source=np.asarray(source_tuple,float); target=np.asarray(target_tuple,float)
    Buse=np.frombuffer(b_bytes,dtype=np.float64).reshape(b_shape)
    ...
    return tuple(map(float,f)),tuple(map(float,r)),float(np.clip(gcf,0,1))
...
def residual_flow(source,target,tiny=1e-6,Bmat=None):
    ...
    f>=0.  A deterministic bounded cache avoids repeatedly solving the same
    training-row linear program inside nested patient folds.  The cache cannot
    create held-out-target leakage because prediction never queries this
    function; it is used only while fitting on rows explicitly supplied as
    training data.
    """
    Buse=np.ascontiguousarray(B if Bmat is None else np.asarray(Bmat,float),dtype=np.float64)
    source=np.asarray(source,float); target=np.asarray(target,float)
    if Buse.shape[0]!=len(source): raise ValueError('B rows must match composition dimension')
    f,r,g=_residual_flow_cached(tuple(map(float,source)),tuple(map(float,target)),Buse.shape,Buse.tobytes(),float(tiny))
    return np.asarray(f,float),np.asarray(r,float),g
...
class GraphFlowModel:
    ...
    def predict(self,X,source):
        ...
        # A zero-edge graph is a valid degenerate structural candidate: it
        # cannot induce graph flow and therefore predicts persistence.  This
        # makes empty-graph controls usable in frozen graph libraries without
        # special-casing them outside the CandidatePredictor contract.
        if self.Bmat.shape[1] == 0:
            return source.copy()
        F=self.predict_flows(X); out=[]
...
def fit_graphflow(X,source,target,alpha=1.0,flow_labels=None,Bmat=None,edges=None,patients=None,patient_balanced=False):
    ...
    if Buse.shape[1] == 0:
        # No edge-flow labels or regression head are required.  The returned
        # model still obeys the normal GraphFlowModel API and deterministically
        # returns source compositions.
        mu=np.nanmedian(X,0); mu=np.where(np.isfinite(mu),mu,0.0)
        scale=np.std(np.where(np.isfinite(X),X,mu),0); scale=np.where(scale>1e-8,scale,1)
        return GraphFlowModel(None,mu,scale,Buse.copy(),edges_tuple,bool(patient_balanced))
    ...
    if flows.ndim == 1: flows=flows[:,None]
...
def source_feature_matrix(pairs,states=None):
    """Return predictor-side features without touching held-out target compositions.

    This helper is deliberately separate from :func:`feature_matrix`.  Candidate
    ``predict`` methods should call it so an inference-time table does not need
    ``target__<state>`` columns at all.  ``target_time`` is permitted because it
    is design metadata, not the held-out biological outcome.
    """
    states=list(STATES if states is None else states)
    missing=[f'source__{s}' for s in states if f'source__{s}' not in pairs.columns]
    if missing: raise KeyError(f'missing source composition columns: {missing}')
    S=pairs[[f'source__{s}' for s in states]].to_numpy(float)
    if 'source_time' not in pairs.columns or 'target_time' not in pairs.columns:
        raise KeyError('source_time and target_time are required predictor metadata')
    ...
    if (dt < -1e-12).any(): raise ValueError('target_time precedes source_time in source_feature_matrix')
    return np.c_[S,dt],S

def feature_matrix(pairs,states=None):
    states=list(STATES if states is None else states)
    X,S=source_feature_matrix(pairs,states=states)
    missing=[f'target__{s}' for s in states if f'target__{s}' not in pairs.columns]
    if missing: raise KeyError(f'missing target composition columns: {missing}')
    return X,S,pairs[[f'target__{s}' for s in states]].to_numpy(float)
