...
def bootstrap_ratio(num,den,patients,B=5000,seed=0,denominator_floor=1e-12):
    """Joint patient bootstrap for a ratio of patient-equal means.

    Numerator and denominator are resampled with the *same* patient indices,
    preserving their patient-level covariance.  A ratio is not silently
    fabricated when the observed or bootstrap denominator is effectively zero;
    callers should use the Movement margin-contrast formulation in that case.
    """
    pats=np.asarray(patients).astype(str); up=np.unique(pats)
    a=np.array([np.asarray(num)[pats==p].mean() for p in up],float)
    b=np.array([np.asarray(den)[pats==p].mean() for p in up],float)
    if not np.isfinite(a).all() or not np.isfinite(b).all():
        raise ValueError('ratio components must be finite')
    if (b < -1e-12).any():
        raise ValueError('ratio denominator contributions must be non-negative')
    den0=float(b.mean())
    if den0 <= float(denominator_floor):
        raise ValueError('ratio denominator is too small to identify Movement ratio; use the margin contrast')
    rng=np.random.default_rng(seed); idx=rng.integers(0,len(up),size=(B,len(up)))
    bd=b[idx].mean(1)
    if np.any(bd <= float(denominator_floor)):
        raise ValueError('bootstrap Movement ratio is undefined in resamples with near-zero denominator; use the margin contrast')
    v=a[idx].mean(1)/bd
    return float(a.mean()/den0),float(np.quantile(v,.025)),float(np.quantile(v,.975))
...
def run_lopo(
    ...
    edges=None,patient_balanced=False,analysis_label='pair_equal_primary',iut_cfg:IUTConfig|None=None
):
    ...
    core=classify_iut(
        ...
        positive_weight_fraction=fold_positive,cfg=iut_cfg or IUTConfig(),
    )
    audit={
        ...
        'core_qualified':core.qualified,'core_evidence_state':core.label,
        'core_axes':dict(core.axes),'evidence_deficit':core.deficits.to_dict(),
        'retention_stability':fold_positive,'core_rule':core.to_dict(),
    }
