...
from .model import STATES, EDGES, incidence, feature_matrix, source_feature_matrix, fit_graphflow, patient_sample_weights
...
class GraphFlowCandidate(CandidatePredictor):
    ...
    def predict(self, pairs: pd.DataFrame):
        ...
        X,S=source_feature_matrix(pairs,states=self.state_names()); p=self._model.predict(X,S)
...
class MeanDeltaCandidate(CandidatePredictor):
    ...
    def predict(self, pairs: pd.DataFrame):
        ...
        _,S=source_feature_matrix(pairs,states=self.state_names()); p=normalize_rows(S+self._delta)
        return self.validate_predictions(p,len(pairs))
