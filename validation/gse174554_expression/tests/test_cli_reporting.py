...
def test_cli_classify_can_emit_legacy_and_both(tmp_path, capsys):
    p = tmp_path / "metrics.json"
    p.write_text(json.dumps(_metrics()))
    assert main(["classify", str(p), "--rule", "legacy"]) == 0
    legacy = json.loads(capsys.readouterr().out)
    assert legacy["label"] == "Qualified" and legacy["total_gates"] == 6
    assert main(["classify", str(p), "--rule", "both"]) == 0
    both = json.loads(capsys.readouterr().out)
    assert both["core"]["label"] == "Qualified" and both["legacy"]["label"] == "Qualified"
